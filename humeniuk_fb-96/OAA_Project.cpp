...
void CheckForTheLastCharacter(string pFullCommand)
{
...
int CreateTableHellper(string pFullCommand, int pNumberCurrentItemStringForCheck, string pNameTable, bool ColumnExistence)
{
    string lComand = "";
    while (pFullCommand[pNumberCurrentItemStringForCheck])                           // пока не конец нашей команды
    {
        if (pFullCommand[pNumberCurrentItemStringForCheck] != ';')
        {
            ...
            if (pFullCommand[pNumberCurrentItemStringForCheck] == ')')
            {
                ...
                AnalizArgumentsOfComandCreate(lComand, pNameTable, &ColumnExistence);
...
{
    bool ColumnExistence = 0, lCheck = 0;
    ...
    while (pFullCommand[pNumberCurrentItemString])// пока не конец буфера
    {
        ...
        if (pFullCommand[pNumberCurrentItemString] != ' ')
        {
            ...
            if (pFullCommand[pNumberCurrentItemString + 1] == ' ' || pFullCommand[pNumberCurrentItemString + 1] == '\0' || pFullCommand[pNumberCurrentItemString + 1] == '(' && !HellperChek(pFullCommand[pNumberCurrentItemString + 1]))
            {
                ...
                if (lCheck == 1){break;}
                if (pFullCommand[pNumberCurrentItemString + 1] == '\0'){return 0;}
                if (pFullCommand[pNumberCurrentItemString + 1] == '(' || pFullCommand[pNumberCurrentItemString + 2] == '(')
                {
                    if (pFullCommand[pNumberCurrentItemString + 1] == '(')
                    {
                        ...
                        CreateTableHellper( pFullCommand, lNumberCurrentItemStringForCheck, lNameTable, ColumnExistence);
                    }
...
int InsertTableHellper(string pFullCommand, int pNumberCurrentItemStringForCheck, string pNameTable)
{
    string lComand = "";
    while (pFullCommand[pNumberCurrentItemStringForCheck])                           // пока не конец нашей команды
    {
        if (pFullCommand[pNumberCurrentItemStringForCheck] != ';')
        {
            ...
            {
                std::cout << lComand << endl;
                AnalizArgumentsOfComandInsert(lComand, pNameTable);
                if (pNameTable == "")
                {
    ...
    while (pFullCommand[pNumberCurrentItemString])                           // пока не конец буфера
    {
        ...
        if (pFullCommand[pNumberCurrentItemString] != ' ')
        {
            ...
            if (pFullCommand[pNumberCurrentItemString + 1] == ' ' || pFullCommand[pNumberCurrentItemString + 1] == '\0' || pFullCommand[pNumberCurrentItemString + 1] == '(' && !HellperChek(pFullCommand[pNumberCurrentItemString + 1]))
            {
                ...
                DatabaseCheckExist(lCheck, lNameTable);
                if (lCheck == 0) { break; }
                if (pFullCommand[pNumberCurrentItemString + 1] == '\0'){ break;}
                if (pFullCommand[pNumberCurrentItemString + 1] == '(' || pFullCommand[pNumberCurrentItemString + 2] == '(')
                {
                    if (pFullCommand[pNumberCurrentItemString + 1] == '(')
                    {
                        lNumberCurrentItemStringForCheck = pNumberCurrentItemString + 2;
                        InsertTableHellper(pFullCommand, lNumberCurrentItemStringForCheck, lNameTable);
                    }
                    else if (pFullCommand[pNumberCurrentItemString + 2] == '(')
                    {
                        lNumberCurrentItemStringForCheck = pNumberCurrentItemString + 3;
...
void AnalizArgumentsOfComandISelect(string pComand)
{
    int lNCISC = 0; //lNumberCurrentItemStringCommand
    ...
    while (pComand[lNCISC + 1])// пока не конец строки
    {
        if (pComand[lNCISC + 1] == 'f' && pComand[lNCISC + 2] == 'r' && pComand[lNCISC + 3] == 'o' && pComand[lNCISC + 4] == 'm')
        {
            ...
            NumberColumn = 0;
            AnaliseHelpper(pComand, lNCISC + 6, &lFullCommand);
            AnaliseFrom(lFullCommand, &lComand, &lNameTable); lComand = "";
            lNCISC = lNCISC + 6 + lNameTable.length();
            NameColumns = 0;
            AnaliseHelpper(pComand, lNCISC + 6, &lFullCommand);
        }
    ...
    while (pFullCommand[pNumberCurrentItemString])                           // пока не конец буфера
    {
        ...
        if (pFullCommand[pNumberCurrentItemString] != ' ')
        {
            ...
            else 
            {
                ...
                AnalizArgumentsOfComandISelect(lFullCommand);
            }
        }
        pNumberCurrentItemString++;
    }
